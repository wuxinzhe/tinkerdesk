 ...
 *
 * 一比一复刻 Hermes tools/patch_parser.py：
...
import type { Hunk, HunkLine, OperationType, PatchApplyResult, PatchOperation } from '@/defines/tools/patch-types'
...
function unifiedDiff(oldContent: string, newContent: string, filePath: string): string {
  ...
  // 简化 unified diff：按行比较（与 Hermes 的 patch_replace diff 语义对齐）
  let i = 0, j = 0
  const body: string[] = []
