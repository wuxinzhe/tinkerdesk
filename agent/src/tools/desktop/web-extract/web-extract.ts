...
import type { WebExtractParams } from '@/defines/tools/params'
import type { DebugCallData, ExtractProvider, ExtractResultItem } from '@/defines/tools/web-extract-types'

/** 提取任意异常的人类可读消息（catch 变量为 unknown，不用 any） */
...
function storeFullText(url: string, content: string): string | null {
  try {
    const cacheDir = path.join(os.tmpdir(), 'showing-web-cache')
...
export class WebExtractTool extends BaseTool<WebExtractParams> {
  readonly id = 'desktop_showing_web_extract'
  readonly name = '网页提取'
  ...
  getSchema(): ToolSchema {
    return {
      type: 'function',
      function: {
        name: 'desktop_showing_web_extract',
        ...
        parameters: {
          ...
          properties: {
            urls: {
              type: 'array',
              description: 'URL strings or search-result objects containing a string url or href field',
              items: { type: 'string' }
            },
            format: {
              type: 'string',
              description: 'Desired output format ("markdown" or "html", optional)',
              enum: ['markdown', 'html']
            },
            char_limit: {
              type: 'integer',
              description: 'Per-page char budget sent to the model (default: 15000). Larger pages truncate.',
              default: DEFAULT_EXTRACT_CHAR_LIMIT
            }
          },
          required: ['urls']
        }
  ...
  async checkAvailability(): Promise<AvailabilityResult> {
    ...
    return { available: true, reason: provider ? `active backend: ${provider.name}` : 'no extract backend' }
  }

  async execute(params: WebExtractParams): Promise<ToolResult> {
    ...
    try {
      ...
      return { ok: !hasError || trimmedResponse.results.some(r => !r.error), data: cleanedResult, error: undefined }
    } catch (exc: unknown) {
