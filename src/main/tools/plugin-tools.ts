 ...
 *
 * 工具：
 * - plugin_install(source)     安装插件（本地路径 或 URL 下载后安装）
 * - plugin_configure(id, values?)  读取配置 schema/当前值；传 values 则保存
 * - plugin_enable(id)          启用（内部走 check 自检）
 * - plugin_list()              列出已安装插件
 * - plugin_uninstall(id)       卸载（删除插件及下载的模型）
 */
...
export const PLUGIN_INSTALL_TOOL_NAME = 'desktop_tinker_plugin_install'
export const PLUGIN_CONFIGURE_TOOL_NAME = 'desktop_tinker_plugin_configure'
export const PLUGIN_ENABLE_TOOL_NAME = 'desktop_tinker_plugin_enable'
export const PLUGIN_LIST_TOOL_NAME = 'desktop_tinker_plugin_list'
export const PLUGIN_UNINSTALL_TOOL_NAME = 'desktop_tinker_plugin_uninstall'
...
/** 安装插件（本地路径或 URL） */
export class PluginInstallTool extends BaseTool {
  constructor(renderer: PromptRenderer, private readonly pluginManager: PluginManager) {
    super(renderer, PLUGIN_INSTALL_TOOL_NAME)
  }
  ...
  async execute(ctx: ToolContext): Promise<ToolResult> {
    ...
    try {
      ...
      if (/^https?:\/\//i.test(source)) {
        // URL → 下载到临时目录后安装
        const tmpDir = join(tmpdir(), `tinkerdesk-plugin-dl-${Date.now()}`)
      ...
      const record = await this.pluginManager.getInstaller().install(srcPath)
      const info = {
        manifest: record.manifest,
        status: (record as unknown as { status(): import('../core/plugin/types').PluginStatus }).status(),
      }
...
/** 读取/保存插件配置 */
export class PluginConfigureTool extends BaseTool {
  constructor(renderer: PromptRenderer, private readonly pluginManager: PluginManager) {
    super(renderer, PLUGIN_CONFIGURE_TOOL_NAME)
  }
  ...
  async execute(ctx: ToolContext): Promise<ToolResult> {
    ...
    try {
      ...
      if (!record) return ToolResult.sync(JSON.stringify({ error: `插件不存在: ${id}` }))
      if (params.values && Object.keys(params.values).length > 0) {
        if (!record.ctx) return ToolResult.sync(JSON.stringify({ error: `插件未初始化: ${id}` }))
        record.ctx.setConfig(params.values as never)
        return ToolResult.sync(JSON.stringify({ ok: true, saved: Object.keys(params.values) }))
      }
      const schema = this.pluginManager.getSchema(id)
      const config = this.pluginManager.getConfig(id)
...
/** 启用插件（内部走 check 自检） */
export class PluginEnableTool extends BaseTool {
  constructor(renderer: PromptRenderer, private readonly pluginManager: PluginManager) {
    super(renderer, PLUGIN_ENABLE_TOOL_NAME)
  }
  ...
  async execute(ctx: ToolContext): Promise<ToolResult> {
    ...
    if (!id) return ToolResult.sync(JSON.stringify({ error: 'id 必填' }))
    try {
      const result = await this.pluginManager.toggle(id, true)
...
/** 列出已安装插件 */
export class PluginListTool extends BaseTool {
  constructor(renderer: PromptRenderer, private readonly pluginManager: PluginManager) {
    super(renderer, PLUGIN_LIST_TOOL_NAME)
  }
...
/** 卸载插件（删除插件及下载的模型） */
export class PluginUninstallTool extends BaseTool {
  constructor(renderer: PromptRenderer, private readonly pluginManager: PluginManager) {
    super(renderer, PLUGIN_UNINSTALL_TOOL_NAME)
  }
