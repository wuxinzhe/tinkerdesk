 ...
 *
 * 复刻 showing-agent HandlebarsPresetModule：
 * 系统预设模块继承此类，通过 PromptRenderer 加载 {id}.hbs 模板。
 * 默认无条件加载（shouldLoad=true），子类可重写。
 */
import type {IDynamicPromptModule, PromptContext} from '../types'
...
export abstract class HandlebarsPresetModule implements IDynamicPromptModule {
  ...
  private buildContextMap(ctx: PromptContext): Record<string, unknown> {
    return {
