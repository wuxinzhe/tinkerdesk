 ...
 *
 * 对应 showing-agent LlmResponse：所有下游层只根据 resType 做分发。
...
/** 空响应 */
export function emptyResponse(): LlmResponse {
  return {resType: RES_EMPTY, text: '', toolCalls: []}
}
...
export function hasToolCalls(response: LlmResponse): boolean {
  return response.resType === RES_TOOL_CALLS
}

/** 是否为有效文本 */
export function isText(response: LlmResponse): boolean {
  return response.resType === RES_TEXT
}
