...
/** 工具名 */
export const TOOL_NAME = 'builtin_tinker_session_search'
...
export class SessionSearchTool extends BaseTool {
  ...
  private doDiscover(args: Record<string, unknown>, profile: string): string {
    ...
    const roleFilter = args.role_filter ? String(args.role_filter) : null
    const sort = args.sort ? String(args.sort) : null

    // 1) 标题匹配优先
    const titleHit = this.sessionService.matchSessionTitle(query, profile)

    // 2) 消息内容 + 标题 LIKE 搜索（role 过滤 + 排序透传）
    const hits = this.sessionService.discoverHits(query, limit, roleFilter, sort, profile)
  ...
  /** SCROLL：消息窗口滚动 */
  private doScroll(args: Record<string, unknown>, ctx: ToolContext, profile: string): string {
  ...
  /** READ：整场会话读取 */
  private doRead(args: Record<string, unknown>, ctx: ToolContext, profile: string): string {
    const sessionId = this.resolveSessionId(args)
    ...
    if (rr.truncated) {
      resp.message = `Session has ${rr.messageCount} messages; tinker first 20 + last 10. Pass around_message_id (any id above) to scroll the middle.`
    }
    return JSON.stringify(resp)
  }

  /** BROWSE：最近会话浏览 */
  private doBrowse(ctx: ToolContext, profile: string): string {
