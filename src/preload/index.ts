...
const api = {
  ...
  // Tool Center（外置工具包：市场查询 + 已装清单 + 安装）
  toolCenter: {
    ...
    install: (pkg: string, registry?: string) => inv('tool:install', { pkg, registry }).then(unwrap),
  },
  ...
  // ── 工具配置（ToolController）──
  tools: {
    list: (payload?: { profile?: string }) => inv('tool-config:list', payload ?? {}).then(unwrap),
